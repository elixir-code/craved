...
for bag_index, bag_file in enumerate(bag_files):
	...
	Desc_tree_clf = DecisionTreeClassifier(random_state=0)
	Desc_tree_clf.fit(X_train,y_train) 
	...
	Desc_tree_f1_score = f1_score(y_test,y_pred,average ='binary')
	#print(rf_f1_score)
	...
	results_file.write("\"{file_name}\",{sample_size},\"{algorithm}\",\"{parameters}\",\"{Desc_tree_f1_score}\",\"{precision_score}\",{recall_score},".format(file_name=bag_files_names[bag_index], sample_size=main.n_samples, algorithm='Desc_tree_clf', parameters=Desc_tree_clf.get_params,Desc_tree_f1_score=Desc_tree_f1_score,precision_score=precision_score,recall_score=recall_score))
